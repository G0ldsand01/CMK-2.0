...
export function LoginPage() {
	...
	const getRedirectUrl = () => {
		...
		if (redirectParam) {
			// Validate that it's a relative URL to prevent open redirect
			if (redirectParam.startsWith('/') && !redirectParam.startsWith('//')) {
				return redirectParam;
			}
		}

		// Use document.referrer if it's from the same origin
		if (document.referrer) {
			try {
				...
				// Invalid referrer URL, fallback to dashboard
			}
		}
				...
				<CardContent className="space-y-6">
					...
					<div className="grid grid-cols-3 gap-3">
						<Button
							...
							disabled={isLoading}
							className="flex flex-col h-auto py-4 gap-2 hover:bg-accent transition-colors">
							<svg
								className="h-5 w-5"
								fill="currentColor"
								viewBox="0 0 24 24"
